...
while (true) {
    ...
    if (!$job) {
        file_put_contents("$log_dir/worker.log", date('Y-m-d H:i:s') . " - Fehler:
ungültiger Job: $job_json\n", FILE_APPEND);
        continue;
    }
    ...
    file_put_contents("$log_dir/simulation.log", $sim_entry, FILE_APPEND);

    // Status-Log
    $status_entry = date('Y-m-d H:i:s') . " - Job für $email bearbeitet.\n";
    file_put_contents("$log_dir/worker.log", $status_entry, FILE_APPEND);
